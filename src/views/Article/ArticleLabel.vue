	...
	<el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="dialogVisible"  :close-on-click-modal="false">
		<el-form :model="dataForm" label-width="80px" :rules="dataFormRules" ref="dataForm" :size="size"
            ...
            <el-form-item label="推荐状态" prop="recommendstatus">
				<el-switch v-model="dataForm.recommendstatus" autocomplete="off" style="float: left;"></el-switch>
			</el-form-item>
		</el-form>
...
export default {
  data () {
    return {
        ...
        columns: [
            ...
            {prop:"name", label:"标签名称", minWidth:120},
            {prop:"recommendstatus",label:"推荐状态",minWidth:120,formatter:this.recommendstatusFormat},
  ...
  methods: {
	...
	findPage: function (data) {
		...
		this.$api.articleLabel.findPage(this.pageRequest).then((res) => {
	...
	handleDelete: function (data) {
        this.$api.articleLabel.batchDelete(data.params).then(data.callback)
    },
    // 显示新增界面
	handleAdd: function () {
    ...
    handleEdit:function (params) {
        ...
        this,dataForm.recommendstatus = true
		
    },
    //推荐状态格式化
    recommendstatusFormat(row, column, cellValue, index){
        return row.recommendstatus == 1 ? '可用' : '禁用';
    },
    // 状态格式化
		...
		this.$refs.dataForm.validate((valid) => {
			if (valid) {
				...
				this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    ...
                    this.dataForm.recommendstatus = this.dataForm.recommendstatus ? 1 : 0
					let params = Object.assign({}, this.dataForm)
					console.log(params)
					this.$api.articleLabel.save(params).then((res) => {
